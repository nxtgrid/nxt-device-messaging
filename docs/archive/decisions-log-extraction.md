# Decisions log — extraction era (archive)

**Do not load this file to implement work.** Chronological journal from 2026-07-27 through
Phase 4 (and plan 002 notes that were appended here before the living log was split).

Living follow-ups: [`docs/decisions-log.md`](../decisions-log.md). Cite this file by path and
date heading when a parked row or ADR points here.

---

The body below is the file as it stood when archived (open/parked tables then, then the
chronological log). Those parked tables are a snapshot; the living list is
...
| ~~**`meter-installs` vendor clients**~~ — ChirpStack `registerDevice` / `setApplicationKeyForDevice` and CALIN v2 repo still imported by hardware provisioning | ~~Metering import (002f)~~ | ✅ ADR-003 §8 (`POST /plugin/provisioning`). Caller rewire remains Metering. |
...
| ~~**Vendor clients are shared with** `meter-installs`**.**~~ Extracting ChirpStack `registerDevice` / `setApplicationKeyForDevice` and the CALIN v2 repo left nxt-backend hardware provisioning without those in-process clients. **Undecided options were:** re-implement thin clients in Metering, expose provisioning endpoints from this service, or share a package | ✅ ADR-003 §8 (`POST /plugin/provisioning`). Plan 001 coupling note. Caller rewire remains Metering. |
