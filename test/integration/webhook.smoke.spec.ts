...
import { createWebhookStore } from '#src/engine/webhook/store.js';
...
describe.skipIf(!shouldRun)('webhook emit → Redis → POST', () => {
  ...
  it('storeAndEmit via base drains and POSTs the WebhookEvent', async () => {
    ...
    try {
      ...
      expect(await redisRepo.client.zcard(webhookRedisKeys.pending())).toBe(0);
    }
    finally {
      ...
      if (pendingIds.length > 0) {
        ...
        multi.del(webhookRedisKeys.pending());
        await multi.exec();
      }
    }
  });
